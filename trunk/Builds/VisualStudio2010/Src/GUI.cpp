...
void GUI::initialize()
{
	_primCreator = new PrimitiveCreator(Rect<int>(0, 0, _bounds.width, _bounds.height / 6), _openGLCanvas);
	_primCreator->initialize();
	addAndMakeVisible(_primCreator);

	_renderSettings = new RenderSettings(Rect<int>(0, _bounds.height / 6, _bounds.width, (int)(_bounds.height * 0.85)), _openGLCanvas);
